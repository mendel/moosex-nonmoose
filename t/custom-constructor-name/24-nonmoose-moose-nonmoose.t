...
with_immutable {
    my $foo = Foo::Moose::Sub->new_instance(name => 'foomoosesub', foo2 => 'FOO2');
    isa_ok($foo, 'Foo');
    ...
    $foo = Foo::Moose->new_instance(name => 'foomoosesub', foo2 => 'FOO2');
    isa_ok($foo, 'Foo');
    isa_ok($foo, 'Foo::Moose');
    is($foo->foo, 'foomoosesub', 'got name from nonmoose constructor');
    is($foo->foo2, 'FOO2', 'got attribute value from moose constructor');
...
with_immutable {
    my $bar = Bar::Moose::Sub->new_instance(name => 'barmoosesub', bar2 => 'BAR2');
    isa_ok($bar, 'Bar');
    ...
    $bar = Bar::Moose->new_instance(name => 'barmoosesub', bar2 => 'BAR2');
    isa_ok($bar, 'Bar');
    isa_ok($bar, 'Bar::Moose');
    is($bar->bar, 'barmoosesub', 'got name from nonmoose constructor');
    is($bar->bar2, 'BAR2', 'got attribute value from moose constructor');
